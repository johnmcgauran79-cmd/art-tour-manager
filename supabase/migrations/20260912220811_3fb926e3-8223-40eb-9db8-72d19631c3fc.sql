...
BEGIN
  ...
  INTO v_backup_hours
  FROM public.backup_runs WHERE status = 'success' AND kind <> 'storage';
...
AS $function$
  ...
  SELECT jsonb_build_object(
    'generated_at', now(),
    'stale_backup_hours', (SELECT (extract(epoch FROM (now() - max(finished_at)))/3600)::numeric(10,1) FROM public.backup_runs WHERE status='success' AND kind <> 'storage'),
