...
BEGIN
  ...
  SELECT 'xero_mismatch:' || m.id, 'xero_reference_mismatch', m.booking_id,
         coalesce(nullif(m.client_name,''), 'Booking'),
         'Booking reference ' || m.booking_reference || ' does not include linked Xero invoice ' || m.xero_invoice_number,
  ...
  WHERE nullif(trim(coalesce(m.booking_reference,'')),'') IS NOT NULL
    ...
    -- the booking reference may list one invoice per passenger; it is fine as long as
    -- one of the listed numbers matches the linked Xero invoice
    AND NOT EXISTS (
      SELECT 1
