-- Update the generate_tour_operation_tasks function to assign based on departments
CREATE OR REPLACE FUNCTION generate_tour_operation_tasks(p_tour_id UUID)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  ...
  v_task_id UUID;
  v_department_name department_name;
BEGIN
    ...
    IF v_reference_date IS NOT NULL THEN
      ...
      IF NOT EXISTS (
        ...
        CASE v_template.category
          WHEN 'booking' THEN
            v_department_name := 'booking'::department_name;
          WHEN 'operations' THEN
            v_department_name := 'operations'::department_name;
          WHEN 'finance' THEN
            v_department_name := 'finance'::department_name;
          WHEN 'marketing' THEN
            v_department_name := 'marketing'::department_name;
          WHEN 'maintenance' THEN
            v_department_name := 'operations'::department_name; -- Map maintenance to operations
          ELSE
            v_department_name := 'operations'::department_name; -- Default to operations
        END CASE;
