...
AS $function$
  ...
  SELECT jsonb_build_object(
    ...
    'http_failure_details_24h', (
      SELECT coalesce(jsonb_agg(x ORDER BY x->>'created' DESC), '[]'::jsonb) FROM (
        SELECT jsonb_build_object(
          'created', r.created,
          'status_code', r.status_code,
          'error', left(coalesce(r.error_msg, coalesce(r.content, '')), 300)
        ) AS x
...
DECLARE
  ...
  v_storage jsonb;
  v_storage_hours numeric;
  v_code jsonb;
  v_code_hours numeric;
  v_mailboxes jsonb;
...
BEGIN
  ...
  SELECT count(*)::int INTO v_http_failures
  FROM net._http_response r
  ...
  INTO v_backup_hours
  FROM public.backup_runs WHERE status = 'success' AND kind IN ('database', 'full');

  SELECT to_jsonb(b) INTO v_storage
  FROM (
    SELECT status, finished_at, size_bytes, destination, error_message
    FROM public.backup_runs
    WHERE kind IN ('storage', 'full')
    ORDER BY finished_at DESC
    LIMIT 1
  ) b;

  SELECT (extract(epoch FROM (now() - max(finished_at))) / 3600)::numeric(10,1)
  INTO v_storage_hours
  FROM public.backup_runs WHERE status = 'success' AND kind IN ('storage', 'full');

  SELECT to_jsonb(b) INTO v_code
  FROM (
    SELECT status, finished_at, size_bytes, destination, error_message
    FROM public.backup_runs
    WHERE kind = 'code'
  ...
  SELECT (extract(epoch FROM (now() - max(finished_at))) / 3600)::numeric(10,1)
  INTO v_code_hours
  FROM public.backup_runs WHERE status = 'success' AND kind = 'code';
  ...
  v_problems := v_problems
    ...
    + CASE WHEN v_storage_hours IS NULL OR v_storage_hours > 192 THEN 1 ELSE 0 END
    + CASE WHEN v_code_hours IS NULL OR v_code_hours > 48 THEN 1 ELSE 0 END
    + CASE WHEN v_http_failures > 0 THEN 1 ELSE 0 END
  ...
  RETURN jsonb_build_object(
    ...
    'http_failures_24h', v_http_failures,
    ...
    'storage_backup', jsonb_build_object(
      'last_run', v_storage,
      'hours_since_success', v_storage_hours,
      'stale_after_hours', 192
    ),
    'code_backup', jsonb_build_object(
      'last_run', v_code,
      'hours_since_success', v_code_hours,
