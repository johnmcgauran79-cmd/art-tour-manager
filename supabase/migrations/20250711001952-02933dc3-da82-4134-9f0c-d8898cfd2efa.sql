...
-- Only exclude 'cancelled' status since host, pending, and all other statuses should count
...
BEGIN
    ...
    IF COALESCE(NEW.activity_id, OLD.activity_id) IS NOT NULL THEN
        ...
        SET spots_booked = (
            ...
            AND b.status != 'cancelled'  -- Only exclude cancelled bookings
        )
...
BEGIN
    ...
    IF OLD.status IS DISTINCT FROM NEW.status THEN
        ...
        -- Include host, pending, and all other statuses except cancelled
        UPDATE activities 
        SET spots_booked = (
            SELECT COALESCE(SUM(ab.passengers_attending), 0)
            FROM activity_bookings ab 
