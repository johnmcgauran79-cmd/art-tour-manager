...
BEGIN
  ...
  v_problems := v_problems
    ...
    + (SELECT count(*)::int FROM jsonb_array_elements(v_mailboxes) e
       WHERE (e->>'enabled')::boolean AND coalesce(e->>'last_status','') NOT IN ('success','completed',''));
...
AS $fn2$
  SELECT jsonb_build_object(
    ...
    'failed_jobs_24h', (
      SELECT coalesce(jsonb_agg(jsonb_build_object('jobname', j.jobname, 'failures', c.cnt, 'message', left(coalesce(c.msg,''),300))), '[]'::jsonb)
      FROM cron.job j
      JOIN LATERAL (
        SELECT count(*)::int AS cnt, max(r.return_message) AS msg
        FROM cron.job_run_details r
        WHERE r.jobid = j.jobid AND r.start_time > now() - interval '24 hours' AND r.status <> 'succeeded'
      ) c ON c.cnt > 0
    ),
    ...
    'mailbox_failures', (
      ...
      WHERE coalesce(m.sync_enabled, false) AND coalesce(r.status,'') NOT IN ('success','completed')
    ),
...
REVOKE ALL ON FUNCTION public.get_system_health_service() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_system_health_service() TO service_role;