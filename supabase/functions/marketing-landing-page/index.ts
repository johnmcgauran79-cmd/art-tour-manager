...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("landing_pages")
      .select(
        "id, slug, title, headline, subheadline, body_html, hero_image_url, fields, field_config, extra_tour_options, room_type_options, hear_about_options, consent_text, thank_you_message, thank_you_heading, submit_button_text, is_active, tour_id, form_type, tour_ids, success_redirect_url, show_country, show_travellers, show_previous_traveller, show_preferred_contact, allow_multiple_tours, brand:brands(name, logo_url, color_primary, color_button, color_button_text, company_website, company_phone)"
      )
      .eq("slug", slug)
      .maybeSingle();
