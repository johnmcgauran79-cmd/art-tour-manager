...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Resolve the entity being notified about (task, personal to-do, or personal note)
    const { data: entity } = isNote
      ? await supabase
          .from("personal_notes").select("id, title").eq("id", entityId).single()
      : isTodo
      ? await supabase
    ...
    const subjectLine =
      body.type === "note_share"
        ? `${actorName} shared a note with you`
        : body.type === "note_mention"
        ? `${actorName} tagged you in a note`
...
<p style="color:#55575d;font-size:15px;line-height:1.6;margin:0 0 18px;"><strong>${actorName}</strong> ${body.type === "note_share" ? "shared the note below with you." : body.type === "note_mention" ? "tagged you in the note below." : body.type === "todo_share" ? "added you to the to-do below." : `${body.type === "mention" ? "mentioned you in a comment on" : body.type === "subtask_assignment" ? "assigned you a subtask on" : body.type === "approval_request" ? "has requested your approval on" : body.type === "approval_decision" ? decisionVerb : "assigned you to"}${body.type === "approval_decision" ? "." : " the task below."}`}</p>
...
<a href="${taskUrl}" style="display:inline-block;background-color:#1a2332;color:#f5c518;text-decoration:none;padding:12px 28px;border-radius:6px;font-size:14px;font-weight:600;">${isNote ? "OPEN NOTES" : isTodo ? "OPEN TO-DO LIST" : "OPEN TASK"}</a>
</td></tr></table>
</td></tr>
