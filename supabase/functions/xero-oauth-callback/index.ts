...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'authorize') {
      ...
      const scopes = 'openid profile email accounting.transactions accounting.transactions.read accounting.contacts.read accounting.contacts accounting.settings offline_access';
