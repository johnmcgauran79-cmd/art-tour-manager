...
export const EmailPassportReportModal = ({
  open,
  onOpenChange,
  tourName,
  onSend,
  isSending,
}: EmailPassportReportModalProps) => {
  const { user } = useAuth();
  ...
  const [subject, setSubject] = useState(`Passport Details Report - ${tourName}`);
  ...
  useEffect(() => {
    setSubject(`Passport Details Report - ${tourName}`);
    setMessage(
      `Dear Team,\n\nPlease find attached the passport details report for ${tourName}.\n\nThis report includes all travel document information for passengers on this tour.\n\nKind regards,\nOperations Team`
    );
  }, [tourName]);
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-4">
          ...
          <div className="text-sm text-muted-foreground">
            <p className="italic">Note: The passport details report will be included in the email body. Recipients can print to PDF using their browser's print function.</p>
          </div>
        </div>
