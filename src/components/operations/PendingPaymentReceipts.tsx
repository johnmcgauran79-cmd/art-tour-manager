      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="border rounded-md">
            <div className="grid grid-cols-[auto_1.2fr_1.6fr_1fr_1fr_1fr] gap-3 items-center px-3 py-2 border-b bg-muted/40 text-xs font-medium text-muted-foreground">
              <Checkbox
                checked={allSelected}
            ...
            <div className="divide-y max-h-[420px] overflow-y-auto">
              {rows.map((r) => {
                ...
                return (
                  <label
                    key={r.id}
                    className="grid grid-cols-[auto_1.2fr_1.6fr_1fr_1fr_1fr] gap-3 items-center px-3 py-2 text-sm hover:bg-muted/30 cursor-pointer"
                  >
                    <Checkbox
                    ...
                    <div className="font-medium">{formatMoney(r.amount, ccy)}</div>
                    <div>{formatMoney(r.invoice_amount_due, ccy)}</div>
                    <div>{r.payment_date ? format(new Date(r.payment_date), "dd/MM/yyyy") : "—"}</div>
                  </label>
                );
              })}
            </div>
