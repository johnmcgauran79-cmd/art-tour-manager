...
export interface ActivityExternalLink {
  id: string;
  activity_id: string;
  label: string;
  url: string;
...
export const useActivityExternalLinks = (activityId: string) => {
  return useQuery({
    queryKey: ['activity-external-links', activityId],
    queryFn: async () => {
      ...
      return data as ActivityExternalLink[];
    },
    enabled: !!activityId,
  });
};

export const useCreateActivityExternalLink = () => {
  const queryClient = useQueryClient();
  const { toast } = useToast();

  return useMutation({
    mutationFn: async (data: { activityId: string; label: string; url: string }) => {
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['activity-external-links', variables.activityId] });
      toast({
...
export const useUpdateActivityExternalLink = () => {
  const queryClient = useQueryClient();
  const { toast } = useToast();

  return useMutation({
    mutationFn: async (data: { id: string; activityId: string; label: string; url: string }) => {
      const { data: link, error } = await supabase
        .from('activity_external_links')
...
export const useDeleteActivityExternalLink = () => {
  const queryClient = useQueryClient();
  const { toast } = useToast();
