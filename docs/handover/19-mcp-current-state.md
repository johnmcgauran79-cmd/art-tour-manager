...
`supabase/functions/mcp/index.ts` exposes ART Admin as an MCP server named `art-tour-manager-wordpress-mcp`, version **2.9.0**, with **150 tool definitions**. Helper implementations live in `src/lib/mcp/tools/` and `src/lib/mcp/wordpress/`.
