...
`supabase/functions/mcp/index.ts` exposes ART Admin as an MCP server named `art-tour-manager-wordpress-mcp`, version **2.10.0**, with **150 tool definitions**. Helper implementations live in `src/lib/mcp/tools/` and `src/lib/mcp/wordpress/`.
...
v2.9.0 added full-field editing: `update_record_fields` (any editable column on tours, activities, hotels), `get_contact_details` and `update_contact` (every contact field; name/email/phone changes return a before/after preview and need `confirm=true`), and `manage_external_link` (add/update/remove links on tours, activities, hotels — e.g. ticket links). Locked fields live in `src/lib/mcp/tools/_fieldUpdate.ts` (`LOCKED_FIELDS`): ids, timestamps, parent tour ids, computed counts, lifetime/latest-tour stats, legacy CRM ids and marketing consent.

v2.10.0: `upload_itinerary_document` also fills the Brochure and Tour Itinerary slots on the Files tab.
