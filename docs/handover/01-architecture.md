...
4. **ART owns its marketing stack.** Campaign sending, audiences, tracking and attribution are built in-house (`marketing_*` tables and `marketing-*` functions). Brevo and Keap remain only as historical/contact-sync surfaces. This is not a third-party ESP integration.
